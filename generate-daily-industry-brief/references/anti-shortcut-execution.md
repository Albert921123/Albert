...
The expansion is a post-filter recovery pass, not a query issued in advance. `record-query --lane expansion` may be persisted only after every candidate from the four baseline lanes has an individual decision (or all four lanes returned zero persisted candidates). It must add both a changed source family and a changed actor class. `close-section` records the machine-generated `post_filter_recovery` object; Agents must copy it unchanged into the ledger. Missing or hand-authored recovery proof blocks render authorization.
