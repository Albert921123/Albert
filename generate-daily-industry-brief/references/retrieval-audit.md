...
For every row below `target_card_count`, `retrieval_proof.post_filter_recovery` is mandatory and hash-bound through the work-state/ledger comparison. It records `required`, `completed`, `initial_candidate_count`, `final_candidate_count`, `initial_review_completed_before_expansion`, `expansion_recorded_at`, `changed_source_families` and `changed_actor_classes`. The expansion must occur after initial screening and must add both a new source family and a new actor class. The ledger's aggregate actor classes must cover every actor class listed in the immutable plan. This prevents a broad first pass followed by aggressive deletion from masquerading as a genuinely quiet board.
