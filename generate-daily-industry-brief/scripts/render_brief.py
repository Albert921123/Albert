...
def main():
    ...
    for row in rows:
        a=section_audit.get(row.get("section_id"),{}); add('<div data-audit-row="%s" role="row"><span>%s</span><span>%s</span><span>%s</span><span>%s</span><span>%s</span><span>%s</span><span>%s</span></div>'%(esc(row.get("section_id")),esc(row.get("label")),a.get("source_family_count",0),a.get("candidate_count",0),a.get("unique_event_count",0),a.get("cross_section_card_count",0),a.get("related_count",0),esc(a.get("status"))))
    add('</div>')
    for row in rows:
