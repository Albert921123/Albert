...
## v1.43 建筑产业锚点、精确排除与审计对账

前八个经营板块新增建筑产业锚点硬门，数科不再收录无建筑产业对象的泛银行科技；拓展阅读固定覆盖政策宏观深读、行业研究、企业项目案例、技术模式观察和趋势风险五条线。每条排除记录必须给出具体代码与原因，页面七列审计必须逐格与检索台账一致。计划触发时间、实际启动时间和延迟秒数也进入正式运行记录。
...
执行程序会拒绝以下情况：未绑定板块画像、来源族未达计划地板、主体或事件覆盖不足、低于目标时没有更换词组和来源族补检、候选仍未逐条决定、工作状态与台账不一致。补检必须在初始候选全部决定后执行，同时新增来源族和主体类型；程序会生成 `post_filter_recovery` 并要求工作状态与台账完全一致，从而阻止“删除弱相关候选后直接判空”。普通日期版 HTML 的结构校验会自动再运行严格台账校验，只有收录项的简化台账不能再绕过正式交付门。真实只有一条合格资讯时仍保留一条；新增规则约束的是检索深度，不是强行凑数。
...
| 编号 | 板块 | 主要关注方向 |
|---|---|---|
| 01 | 数科 | 面向建筑、建材、房地产、基础设施和工程供应链的数字交易、产业金融、结算、项目资金监管与经营数据能力 |
...
| 16 | 拓展阅读 | 建筑产业政策宏观深读、行业研究、企业项目案例、技术模式观察与趋势风险专题 |
